//-----------------------------------------------------------------------------
// F342DC_USB0_InterruptServiceRoutine.c
//-----------------------------------------------------------------------------
...
U8 USB0_STATE;               // Holds the current USB State
                                       // def. in F342DC_USB0_InterruptServiceRoutine.h
...
void Fifo_Read (U8, U16, U8 *);
                                       ...
                                       // of Endpoint fifos
void Fifo_Write_Foreground (U8, U16, U8 *);
                                       // Used for multiple byte writes
                                       // of Endpoint fifos in foreground
void Fifo_Write_InterruptServiceRoutine (U8, U16,
										 U8 *);
   ...
   if (EP_STATUS[0] == EP_RX)          // See if endpoint should transmit
   {
      ...
      if (ControlReg & rbOPRDY)        // Verify packet was received
      {
         ControlReg = rbSOPRDY;
   		if (DATASIZE >= EP0_PACKET_SIZE)
   		{
            Fifo_Read(FIFO_EP0, EP0_PACKET_SIZE, (U8*)DATAPTR);
            // Advance data pointer
            DATAPTR  += EP0_PACKET_SIZE;
            // Decrement data size
            DATASIZE -= EP0_PACKET_SIZE;
   		...
   		else
   		{
            ...
            Fifo_Read (FIFO_EP0, DATASIZE, (U8*) DATAPTR);
...
//
//-----------------------------------------------------------------------------
void Fifo_Read (U8 addr, U16 uNumBytes,
               U8 * pData)
{
   S16 i;
...
void Fifo_Write_Foreground (U8 addr, U16 uNumBytes,
                    U8 * pData)
{
   S16 i;
...
void Fifo_Write_InterruptServiceRoutine (U8 addr,
                                         U16 uNumBytes,
                                         U8 * pData)
{
   S16 i;
