...
U8 SWITCH_1_STATE = 0;      // Indicate status of switch
...
//-----------------------------------------------------------------------------
INTERRUPT_PROTO(Adc_ConvComplete_ISR, 10);
...
void Port_Init(void)
{
    P1MDOUT |= 0xFF;                    // Port 1 pins 0-7 set high impedence
    P2MDOUT |= 0xFF;                    // Port 2 pins 0-7 set high impedence
    XBR0     = 0x00;
...
// to 2.5 in single-ended mode.  Also enables the internal voltage reference.
//
//-----------------------------------------------------------------------------
void ADC0_Init (void)
{
   ...
   AMX0P = 0x0D;                       // P2.5 is the positive input
   AMX0N = 0x1F;                       // GND is the negative input

   // ADC conversion clock <= 3MHz
   ADC0CF = (SYSCLK/ADC_CLOCK) << 3;   

   ADC0CF |= 0x04;                     // Make ADC0 left-justified
